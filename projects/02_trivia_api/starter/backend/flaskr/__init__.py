...
def create_app(test_config=None):
  ...
  '''
    ...
    def categories():
        ...
        for category in categories:
          return jsonify({'categories': {category.id: category.type}}), 200
  ...
  '''
    ...
    def create_question():
        ...
        error = []
        if not isinstance(new_question.question, str):
            error.append("Question should be a string")
        if not isinstance(new_question.answer, str):
            error.append("Answer should be a string")
        if not isinstance(new_question.category, int):
            error.append("Category should be an integer")
        if not isinstance(new_question.difficulty, int):
            error.append("Difficulty should be an integer")
        if error:
            abort(400, error)
